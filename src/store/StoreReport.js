...
const StoreReport = () => {
  ...
  const filterOrders = (type) => {
    if (type === 'processed') {
      setFilteredOrders(orders.filter(order => order.status === 'processed'));
    } else if (type === 'completed'){
      setFilteredOrders(orders.filter(order => order.status == 'completed'));
    } else if (type === 'all') {
      setFilteredOrders(orders.filter(order => order.status !== 'all'));
    }
  };
...
const ProceedStore = async (orderid) => {
  ...
  return (
    <View style={styles.maincontainer}>
      ...
      <TouchableOpacity
          ...
          >
            <Text style={styles.addButtonText}>All Order List</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.addButtonprocessed]}
            onPress={() => filterOrders('processed')}
          >
            <Text style={styles.addButtonText}>Orders In Process</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.addButton]}
            onPress={() => filterOrders('completed')}
          >
            <Text style={styles.addButtonText}>Completed</Text>
          </TouchableOpacity>
        </View>
            ...
            >
              ...
              <View style={styles.orderHeader}>
                <Text style={styles.orderHeaderText}>
                 <Text style={styles.orderHeaderMain}>Order Number:</Text>  {order.orderNumber} | <Text style={styles.orderHeaderMain}>Status:</Text> <Text style={[
  ...
  </Text> |<Text style={styles.orderHeaderMain}> Date:</Text>{' '}
                  {order.created_at}
                </Text>
              </View>
 ...
 <View style={{ flexDirection: 'row' }}>
                <Button mode="outlined" onPress={() => generatePDF(order)} style={{ marginTop: 10 }}>
                 ...
                 <TouchableOpacity
               style={[order.status == "processed"? styles.addButton: styles.disableButton]}
