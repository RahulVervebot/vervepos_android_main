...
// ✅ Reusable date selector (no-paper) – same format/offset logic
...
const PaymentReport = () => {
  ...
  // Initial timezone + default to Today (same formatting as your example)
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // 2) compute now same way: UTC + offset hours
  ...
  return (
    <Provider>
      <View style={[
        ...
        {/* 🔁 Replaced the old Dialog/FlatList/DateTimePicker with the reusable selector */}
        ...
        {/* From/To boxes (same UI as your first example) */}
        {startDate && endDate && (
          <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'center', marginTop: 20 }}>
        ...
        {endDate && startDate ? (
          <Button
            mode="contained"
            onPress={() => {
              // 🚫 No API changes — just pass your params as before
