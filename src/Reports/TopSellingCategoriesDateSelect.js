// screens/TopSellingCategoriesPaymentReport.js (or keep your original name)
...
const PaymentReport = () => {
  ...
  const [timezone, setTimezone] = useState('America/New_York'); // default
  const [numCategories, setNumCategories] = useState('');

  // Same alias map you used
  const ZONE_ALIASES = {
  ...
  // Initialize tz + default to Today (UTC+offset) with exact formatting
  useEffect(() => {
    const init = async () => {
      try {
        ...
        console.log('Init tz/dates error', e);
      }
    };
    init();
  }, []);
  ...
  return (
    <Provider>
      <View style={[
        ...
        {/* 🔁 Drop Dialog/FlatList/DateTimePicker; use the reusable selector */}
        ...
        {endDate && startDate ? (
          <Button
            mode="contained"
            onPress={() => {
              // 🚫 No API changes — same params
              console.log('startDate In Navigation : ', startDate);
              console.log('endDate in Navigation: ', endDate);

              navigation.navigate('TopSellingCategories', {
                startDate,
                endDate,
                numCategories: numCategories || '10',
              });
            }}
