...
import { fetchManagerDataForRequest, postApprovalRequest,getTotalAllocationCurrentWeek,AllocatedBudgetAllVendorCurrentWeek } from '../../functions/VendorAccess/function';
import AsyncStorage from '@react-native-async-storage/async-storage';
const VendorManagerRequest = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      setTimeout(() => {
        setWeekly(); // Apply weekly filter after 1s
      }, 1000);
    };
  ...
  return (
   ...
   <Text variant="titleMedium" style={styles.titleTextWhite}>Remaining Budget(loading)</Text>
    ...
    <Text style={styles.selectedRangeText}>
      Selected Range: {startDate ? startDate.toLocaleDateString() : 'All'} - {endDate ? endDate.toLocaleDateString() : 'All'}
    </Text>
      ...
      data.map((manager, index) => (
        <Card key={index} style={styles.card}>
          <Card.Content>
            ...
            <Text style={styles.cardText}>Vendor Name: {manager.vendor_name}</Text>
...
export default VendorManagerRequest;
