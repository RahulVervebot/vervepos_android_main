...
const ManagerRequest = () => {
  ...
  const initializeData = useCallback(async () => {
    ...
    start.setDate(currentDate.getDate() - daysUntilMonday);
    start.setHours(0, 0, 0, 0);
    const end = new Date(start);
    end.setDate(start.getDate() + 6);
    end.setHours(23, 59, 59, 999);
    const startFormatted = `${String(start.getMonth() + 1).padStart(2, '0')}-${String(start.getDate()).padStart(2, '0')}-${start.getFullYear()}`;
    const endFormatted = `${String(end.getMonth() + 1).padStart(2, '0')}-${String(end.getDate()).padStart(2, '0')}-${end.getFullYear()}`;
    setStartDate(start);
    setEndDate(end);
    setStartDateFormatted(startFormatted);
    setEndDateFormatted(endFormatted);
    try {
      const allocationDetails = await getTotalAllocationCurrentWeek();
      if (allocationDetails?.status === 'success' && Array.isArray(allocationDetails.data) && allocationDetails.data.length > 0) {
        ...
        const progressValue = allocated_amount ? remaining_allocated_amount / allocated_amount : 0;
...
useFocusEffect(
  useCallback(() => {
    const load = async () => {
      await initializeData(); // Set weekly dates
    };
    load();
  }, [])
);
...
}, [startDate, endDate, accessToken, storeUrl]);


const fetchData = async (customStartDate, customEndDate) => {
  setLoading(true);
  const fetchedData = await fetchManagerDataForRequest(
    accessToken,
    storeUrl,
    (result) => {
      setAllData(result);
      filterDataByDateRange(customStartDate, customEndDate, result);
    },
    setLoading(false),
  );
};
  ...
  const handleApproval = async (requestId, approvalType, amount,warehouseId) => {
    const approvalData =
    ...
    await postApprovalRequest(accessToken, storeUrl, approvalData, warehouseId, setLoading);
    setModalVisible(false); // Close the modal after submitting
    setRejectModalVisible(false);
    setRejectReason('');
    const updatedData = await fetchData(); // Get latest data
    filterDataByDateRange(startDate, endDate, updatedData);
  ...
  const filterDataByDateRange = (startDate, endDate, source = allData) => {
    if (!startDate || !endDate) return setData(source);
        ...
        {data.length > 0 ? (
          data.map((manager, index) => (
            <Card key={index} style={styles.card}>
              <Card.Content>
                <Text style={styles.cardText}>Warehouse: {manager.warehouseName}</Text>
                ...
                <View style={styles.approvebuttonContainer}>
                  {manager.request_status == 'pending' && (
                    <View style={styles.buttonContainer}>
                      <Button
                        mode="contained"
                        onPress={() => handleApproval(manager.request_id, 'approved', manager.amount_requested,manager.warehouseId)}
                      ...
                      <Button
                        ...
                        onPress={() => {
                          setSelectedRequestId(manager.request_id);
                          setRejectModalVisible(true); // Open the modal for partial approval
                        }}
                        style={styles.rejetButton}
                      >
                        Reject
                      </Button>
                    </View>
...
<Modal visible={isRejectModalVisible} animationType="slide" transparent={true}>
            ...
            <Card style={styles.modalCard}>
              <Card.Content>
                ...
                <View style={styles.modalButtonContainer}>
                  <Button
                    mode="contained"
                    onPress={() => handleApproval(selectedRequestId, 'reject')}
                    style={styles.modalApproveButton}
                  >
                    Submit
                  </Button>
                  <Button mode="outlined" onPress={() => setRejectModalVisible(false)} style={styles.modalCancelButton}>
                    Cancel
                  </Button>
                </View>
              </Card.Content>
