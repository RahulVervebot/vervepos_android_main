...
import {
  ...
  AllocatedBudgetAllVendor,
  getTotalAllocationDetail,
  distributeAllocatedAmountDepartmentNextWeek
} from '../../functions/DepartmentAccess/function_dep';
import { useFocusEffect } from '@react-navigation/native';
const DepartmentBudgetNextWeek = () => {
  ...
  setActiveDepartment(alldepartment.data);
    ...
    const ActiveDept = await getTotalAllocationDetail();
 ...
 // const activeCategories = categories.categories || [];
  ...
  const handleAllocateSubmit = async () => {
    if (selectedCategory && price && selectedCategoryid) {
      ...
      const response = await distributeAllocatedAmountDepartmentNextWeek(
        selectedCategory,
        parseFloat(price),
        selectedCategoryid,
        currentAllocationId
     ...
     // const response = await distributeAllocatedAmountDepartmentNextWeek(selectedCategory, parseFloat(price), selectedCategoryid, selectedCategoryallocationId);
            ...
            {loading ? (
              ...
              <Card.Content>
                <Text variant="titleMedium" style={styles.titleTextWhite}>
                 From : {startDateFormatted} to {endDateFormatted} | {selectedWarehouse} | {selectedWarehouseId}
                </Text>
                 <Text variant="titleMedium" style={{ color: 'white', fontWeight: "700", fontSize: 20, marginTop: 10, marginBottom: 10}}>
               Budget:   $
                  {selectedWarehouseBudget?.allocated_amount
                    ? selectedWarehouseBudget.allocated_amount.toFixed(2)
                    : 0} | Used Budget: ${selectedWarehouseBudget?.remainingPOBalance ? (selectedWarehouseBudget.allocated_amount - selectedWarehouseBudget.remaining_allocated_amount).toFixed(2): 0}
                </Text>
                <ProgressBar progress={totalspend} style={styles.progressBar} />
                <View>
                   <Text variant="titleMedium" style={{ color: 'white', fontWeight: "700", fontSize: 20, marginTop: 10, marginBottom: 10}}>
                   Budget in Hand ($
                    {selectedWarehouseBudget?.remaining_allocated_amount
                      ? selectedWarehouseBudget.remaining_allocated_amount.toFixed(2)
                      : 0}
                    )
                  </Text>
                </View>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.tableContainer}>
            <View style={[styles.tableRow, styles.headerRow]}>
              <Text style={[styles.tableCell, styles.headerCell]}>Department Name</Text>
              ...
              <Text style={[styles.tableCell, styles.headerCellCentered]}>Alliance</Text>
...
>
  <Text style={styles.popbuttonText}>Add Alliance</Text>
</TouchableOpacity>
: <Text style={styles.popbuttonText}></Text>}
                  </View>
                  <View style={styles.tableCellCentered}>
...
>
  <View style={{ padding: 20, flex: 1 }}>
    <ScrollView>
      ...
      <Text variant="titleMedium" style={{ marginBottom: 10 }}>Select Alliances:</Text>
...
export default DepartmentBudgetNextWeek;
