...
import {
  ...
  getVendorBudgetCurrentWeek, UpdatePOVendor,fetchVendorDetailsForVendor
...
const [isItemsModalVisible, setIsItemsModalVisible] = useState(false);
  ...
  const updateVendorOrder = async (invoiceKey) => {
    ...
    const updatedItems = data
      .filter((item) => `${item.vendorName} - ${item.invoiceNumber}` === invoiceKey)
      .map((item, idx) => ({
        ...item,
  ...
  const handleQtyChange = (text, invoiceKey, idx) => {
    ...
    setData((prevData) =>
      prevData.map((item, itemIdx) => {
        if (`${item.vendorName} - ${item.invoiceNumber}` === invoiceKey && itemIdx === idx) {
  ...
  const loadVendorDetails = async (vendorName) => {
    ...
    const itemsForSelectedVendor = fetchedVendors.filter(
      (vendor) => vendor.vendorName === vendorName // Use vendorName instead of selectedVendor
    );
  ...
  const handleAddSelectedItems = () => {
    ...
    setData((prevData) => {
        ...
        selectedItemsData.forEach((selectedItem) => {
            ...
            if (existingIndex >= 0) {
                updatedData[existingIndex].invQty = parseFloat(updatedData[existingIndex].invQty || 0) + 1;
                updatedData[existingIndex].totalPrice = updatedData[existingIndex].invCaseCost * updatedData[existingIndex].invQty;
            } else {
                updatedData.push({
                    ...selectedItem,
                    invQty: 1,
    ...
    <View style={styles.maincontainer}>
      ...
      <ScrollView style={styles.container}>
        {Object.keys(groupedData).length > 0 ? (
          Object.keys(groupedData).map((invoiceKey, index) => {
            const items = groupedData[invoiceKey];
            const filteredItems = items.filter((rowItem) => {
                ...
                <Card.Content>
                  <View style={styles.vendorheader}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => setSearchQueries({ ...searchQueries, [invoiceKey]: text })}
                    />
                  </View>
                  ...
                  {filteredItems.map((rowItem, idx) => (
                    <View key={idx} style={styles.tableRow}>
                      <Text style={styles.tableCell}>{rowItem.posDepartment}</Text>
                      ...
                      <TextInput
                        ...
                        value={editedQuantities[`${invoiceKey}-${idx}`] ?? rowItem.invQty?.toString() ?? ""}
                        onChangeText={(text) => handleQtyChange(text, invoiceKey, idx)}
                      />

                      <Text style={styles.tableCell}>
                        {(
                          (parseFloat(rowItem.invCaseCost) || 0) *
                          (parseFloat(editedQuantities[`${invoiceKey}-${idx}`] ?? rowItem.invQty) || 0)
                        ).toFixed(2)}
                      </Text>
                      {/* <Text style={styles.tableCell}>{rowItem.invQty}</Text>
                    <Text style={styles.tableCell}>{rowItem.totalCost}</Text> */}
                    </View>
                  ))}
                  {/* Footer info */}
...
<View style={styles.alltablebutton}>
      ...
      {loadingItems ? 
       <ActivityIndicator size="large" color="#0000ff" />
        :
                   <TouchableOpacity
  style={[styles.searchvendorbutton]}
  onPress={() => loadVendorDetails(invoiceKey.split(" - ")[0])} // Extract vendorName from invoiceKey
>
  <Text style={styles.buttonText}>Add Items</Text>
                  ...
                  <Button
                    ...
                    style={{ marginTop: 10, backgroundColor: "#2C62FF" }}
                  >
                    Update PO
                  </Button>
                  </View>
          ...
          <Text style={styles.noRequestText}>Loading</Text>
        )}
      </ScrollView>
           <Modal
