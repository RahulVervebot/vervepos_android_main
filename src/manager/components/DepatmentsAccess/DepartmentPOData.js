...
import {
  ...
  fetchCategoryProducts, // We'll call this inside our wrapper function
...
import { RNCamera } from 'react-native-camera'; // for barcode scanning
import DepartmentGroupedTable from '../DepartmentGroupedTable';
import ManualItemAdder from '../ManualItemAdder';
const DepartmentPOData = () => {
  ...
  // Track the full weekly allocation (the entire object)
  const [weekAllocation, setWeekAllocation] = useState(null);
  ...
  const [departmentRemainingMap, setDepartmentRemainingMap] = useState({});
  const [departmentBudgetMap, setDepartmentBudgetMap] = useState({});
  ...
  // ─────────────────────────────────────────────────────────────
  //  Initialize data - get the total weekly allocation
  // ─────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    const fetchRemainingForDepartments = async () => {
      const uniqueDepartments = [
        ...new Set(poTableData.map((item) => item.departmentName)),
      ];

      const resultMap = {};
      const totalmap = {};
      for (const deptName of uniqueDepartments) {
        const response = await getSingleDepartmentCurrentWeek(deptName);
        if (
          response &&
          ...
          const deptAlloc = response.data[0].department_allocations[0];
          resultMap[deptName] = deptAlloc.departmentRemainingAmount;
          totalmap[deptName] = deptAlloc.departmentAllocatedAmount;
        }
      }

      setDepartmentRemainingMap(resultMap);
      setDepartmentBudgetMap(totalmap);
    };

    if (poTableData.length > 0) {
      fetchRemainingForDepartments();
    }
  }, [poTableData]);

  const handleManualAdd = (item) => {
    setPoTableData((prev) => [...prev, item]);
  };
      ...
      try {
        ...
        if (
          ...
          if (firstRecord.department_allocations) {
            ...
            console.log("departmentAllocations:",departmentAllocations);
          }
  ...
  // ─────────────────────────────────────────────────────────────
  //  Fetch vendor list + set allocation data
  // ─────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        // If you also wanted to fetch total allocation here, you can,
        // but we already do it above. It's optional:
        const allocationDetails = await getTotalAllocationCurrentWeek();
        if (
          allocationDetails &&
          allocationDetails.status === 'success' &&
  ...
  // ─────────────────────────────────────────────────────────────
  //  Filter the vendor names as user types
  // ─────────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────────
  //  Searching Items by Vendor (old approach)
  // ─────────────────────────────────────────────────────────────
  const loadVendorDetails = async () => {
    setLoadingItems(true);
    const fetchedVendors = await fetchStockUpdate();
  ...
  const handleAddSelectedAllItems = () => {
    ...
    setPoTableData((prevData) => {
      const updatedData = [...prevData];
      selectedItemsData.forEach((selectedItem) => {
        const numericPosUnitCost = parseFloat(selectedItem.invCaseCost) || 0;
...
setItemLoading(false);
  ...
  // ─────────────────────────────────────────────────────────────
  //  Add item from API search or scanned barcode to the table
  // ─────────────────────────────────────────────────────────────

  const addSearchItemToTable = async (selectedItem) => {
    const numericPosUnitCost = parseFloat(selectedItem.posUnitCost) || 0;
    const numericInvCaseCost = parseFloat(selectedItem.invCaseCost) || 0;
    const depatmentname = selectedItem.departmentName;

    const vendorname = selectedItem.vendorName
  ...
  const handleManualQtyChange = (barcode, text) => {
      ...
      prevData.map((item) => {
        if (item.barcode === barcode) {
          return {
            ...
            qty: newQty,
            unitQty: 0,
            totalPrice: (parseFloat(item.invCaseCost) || 0) * newQty,
          };
        }
        return item;
      })
  ...
  const updateQty = (barcode, increment) => {
    setPoTableData((prevData) => {
      return prevData.map((item) => {
        if (item.barcode === barcode) {
          const newQty = Math.max(1, (parseInt(item.qty) || 0) + increment);
         const updatedTotal = (parseFloat(item.invCaseCost) || 0) * newQty;
          return {
            ...item,
            qty: newQty,
            unitQty: 0,
  ...
  const handleManualUnitQtyChange = (barcode, text) => {
    let newQty = parseInt(text, 10);
    if (isNaN(newQty) || newQty < 1) newQty = 0;
    ...
    setPoTableData((prevData) =>
      prevData.map((item) => {
        if (item.barcode === barcode) {
          const updatedTotal =
          newQty > 0
            ? (parseFloat(item.posUnitCost) || 0) * newQty
            : (parseFloat(item.invCaseCost) || 0) * 1;
          return {
            ...item,
            unitQty: newQty,
            qty: 0,
  ...
  const updateUnitQty = (barcode, increment) => {
    setPoTableData((prevData) => {
      return prevData.map((item) => {
        if (item.barcode === barcode) {
          const newQty = Math.max(0, (parseInt(item.unitQty) || 0) + increment);
        //  const updatedTotal = (parseFloat(item.invCaseCost) || 0) * newQty;
          const updatedTotal =
          newQty > 0
            ? (parseFloat(item.posUnitCost) || 0) * newQty
  ...
  const handleRemoveItem = (barcode) => {
    setPoTableData((prevData) => prevData.filter((item) => item.barcode !== barcode));
  };

  // ─────────────────────────────────────────────────────────────
  //  Vendor selection + budget
  // ─────────────────────────────────────────────────────────────
  ...
  const totalQty = poTableData.reduce(
  ...
  const generatePDF = async () => {
    ...
    // Send data to your backend (createDepartmentDetailsOrder)
    const accountTransfer = parseFloat(totalInvoiceValue) - parseFloat(totalCash || 0);
    const totalcash = parseFloat(totalCash || 0);
    // Ensure positive values and send additional data
    const dataresponse = await createDepartmentDetailsOrder(poTableData, totalcash, accountTransfer);
    //  const dataresponse = await createDepartmentDetailsOrder(poTableData);
    ...
    if (
      ...
      Alert.alert('Error', dataresponse.error.message);
      setPDFLoading(false);
      setCasheModalVisible(false);
      return;
    }
    ...
    <body>
      ...
      <h4>${startDateFormatted} to ${endDateFormatted}</h4>
      <table>
        <thead>
          <tr>
            <th>Name</th>
    ...
    setSelectedModalItems((prevSelected) => {
      ...
      if (prevSelected.includes(barcode)) {
        ...
        updatedRemainingAmount =
          parseFloat(updatedRemainingAmount) + (parseFloat(unitCost) || 0);
      } else {
        // Otherwise, select it
  ...
  const handleApiSearch = async (query) => {
    try {
      ...
      const response = await fetch(`${storeUrl}/category-management/search/?q=${query}`, {
        method: 'GET',
        headers: {
  ...
  const onBarCodeRead = async (event) => {
    const scannedCode = event.data;
      const isValidUPC = (scannedCode) => {
    // Remove leading zeros
    let cleanedBarcode = scannedCode;

    if (scannedCode.startsWith('0')) {
      cleanedBarcode = scannedCode.slice(1);
     ...
     if (isValidUPC(scannedCode)) {
      console.log('The barcode is UPC-A.', scannedCode.slice(1));
       setCameraVisible(false);
      try {
        setSelectedBarcode(scannedCode.slice(1));
        setBarcodeModalVisible(true);
      } catch (error) {
        console.error("Scanning error:", error);
      }
    } else {
      console.log('The barcode is not UPC-A.', scannedCode);
   if (scannedCode) {
      setCameraVisible(false);
      try {
        console.log("Scanned Barcode:", scannedCode);
        setSelectedBarcode(scannedCode);
        setBarcodeModalVisible(true);
      } catch (error) {
        console.error("Scanning error:", error);
      }
  ...
  // We'll wrap fetchCategoryProducts in our own function that takes deptId
  // so we can pass it as `fetchItemsForDepartment` to the selector.
  const fetchItemsForDepartment = async (deptId) => {
    try {
      const items = await fetchCategoryProducts(deptId); // calls your function_dep code
      console.log("category items: ", deptId);
      return items;
    } catch (err) {
  ...
  // Once the user picks items from DepartmentItemSelector, add them to the table
    ...
    setPoTableData((prevData) => {
      ...
      selectedItems.forEach((selected) => {
        ...
        if (existingIndex >= 0) {
          ...
          updated.push({
            ...selected,
            totalPrice: (selected.invCaseCost || 0) * selected.qty,
            departmentRemainingAmount: selected.departmentRemainingAmount || 0,  // <-- Add this // <-- Add this
          });
        }
      });
      return updated;
  ...
  // Similarly for VendorItemSelector

  const fetchItemsForVendor = async (vendorName) => {
    try {
      const allVendorItems = await handleVendorSearch(vendorName);
  ...
  const updateDepartmentRemaining = () => {
    ...
    poTableData.forEach((item) => {
      ...
      if (!departmentTotals[dept]) {
        departmentTotals[dept] = { total: 0, originalRemaining: item.departmentRemainingAmount || 0 };
      }

      departmentTotals[dept].total += itemTotal;
    });

    const updated = poTableData.map((item) => {
      const deptInfo = departmentTotals[item.departmentName];
      const updatedRemaining = deptInfo.originalRemaining - deptInfo.total;
      return {
        ...item,
        departmentRemainingAmount: updatedRemaining.toFixed(2),
      };
    });

    setPoTableData(updated);
  };
      ...
      selectedItems.forEach((item) => {
        ...
        if (existingIndex >= 0) {
          ...
          updated.push({
            ...
            departmentRemainingAmount: item.departmentRemainingAmount || 0,  // <-- Add this
            departmentId: item.departmentId || 0,
            departmentAllocationId: item.departmentAllocationId || 0
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ─────────────────────────────────────────────────────────
          Search or scan items at the top
      ───────────────────────────────────────────────────────── */}
                ...
                {searchLoading ? (
                  ...
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      ...
                      >
                        <Text style={{ padding: 10 }}>{item.posName} - {item.barcode}</Text>
                      </TouchableOpacity>
                    )}
      ...
      {/* ─────────────────────────────────────────────────────────
          Buttons for "Search by Department" and "Search by Vendor"
      ───────────────────────────────────────────────────────── */}
      ...
      <View style={styles.Rowvendor}>
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={{ padding: 10, backgroundColor: "#d9ecfe", margin: 10, color: "#2C62FF" }}>From: {startDateFormatted} to: {endDateFormatted}</Text>
...
>
  <Text style={{ color: '#fff' }}>Add Manual Item</Text>
      ...
      <ScrollView nestedScrollEnabled={true}>
        <DepartmentGroupedTable
          poTableData={poTableData}
          updateQty={updateQty}
          updateUnitQty={updateUnitQty}
          handleManualQtyChange={handleManualQtyChange}
          handleManualUnitQtyChange={handleManualUnitQtyChange}
          handleRemoveItem={handleRemoveItem}
          departmentRemainingMap={departmentRemainingMap} // 👈 pass it here
          departmentBudgetMap={departmentBudgetMap}
        />
      </ScrollView>
      ...
      <View style={styles.buttonsRow}>
        {pdfloading ? (
          <TouchableOpacity style={[styles.generatepo]} onPress={() => {
            if (appType === 'warehouse') {
              setCasheModalVisible(true);
            } else {
              generatePDF(); // Replace with your actual PDF function
            }
          }}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalView}>
            <Text style={styles.modalCashTitle}>Account Transfer {totalInvoiceValue.toFixed(2) - totalCash}</Text>
            ...
            <View style={styles.buttonRow}>
              <TouchableOpacity style={styles.confirmButton} onPress={generatePDF}>
                {pdfloading ? (
                  <ActivityIndicator size="large" color="#0000ff" />
                ) : (
...
export default DepartmentPOData;
