...
const [selectedBarcode, setSelectedBarcode] = useState('');
    ...
    const fetchRemainingForDepartments = async () => {
     ...
     const totalmap ={};
      for (const deptName of uniqueDepartments) {
        const response = await getSingleDepartmentNextWeek(deptName);
        if (
          response &&
    ...
    const initializeData = async () => {
      ...
      try {
        ...
        if (
          ...
          const { allocated_amount, remaining_allocated_amount } = firstRecord;
  ...
  const depatmentname = selectedItem.departmentName;
   ...
   try {
     const response = await getSingleDepartmentNextWeek(depatmentname);
     if (
       response &&
       response.status === "success" &&
  ...
  const updateQty = (barcode, increment) => {
    setPoTableData((prevData) => {
      return prevData.map((item) => {
        if (item.barcode === barcode) {
          const newQty = Math.max(1, (parseInt(item.qty) || 1) + increment);
 ...
 console.log("poTableData:",poTableData);
    // Send data to your backend (createNextWeekOrder)
    ...
    const dataresponse = await createNextWeekOrder(poTableData,totalcash,accountTransfer);


  //  const dataresponse = await createNextWeekOrder(poTableData);
  ...
  const onBarCodeRead = async (event) => {
    ...
    // ───────────────────────────────────────────────────────────── scanned with barcode without compare
    // const onBarCodeRead = async (event) => {
    //   const scannedCode = event.data;
    //   if (scannedCode) {
    //     setCameraVisible(false);
    ...
    //       const response = await fetch(`${storedurl}/category-management/search/?q=${scannedCode}`, {
    //         method: 'GET',
    //         headers: {
    //           Authorization: `Bearer ${storedtoken}`,
    //           'Content-Type': 'application/json',
    ...
    // };
  // ─────────────────────────────────────────────────────────────
  //  Department + Vendor Item Selectors
  // ─────────────────────────────────────────────────────────────
  ...
  // Once the user picks items from DepartmentItemSelectorNextWeek, add them to the table
    ...
    <View style={styles.container}>
      ...
      <View style={styles.Rowvendor}>
        <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
          ...
          {/* DepartmentItemSelectorNextWeek usage */}
